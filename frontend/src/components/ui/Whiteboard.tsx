...
const Whiteboard: React.FC<WhiteboardProps> = ({ state, onUpdate }) => {
  ...
  const updateWhiteboardState = useCallback((newElements: DrawingElement[]) => {
    const newState: WhiteboardState = {
      ...
      metadata: {
        totalStrokes: newElements.filter(el => el.type === 'line').length,
        totalShapes: newElements.filter(el => el.type !== 'line').length,
        lastTool: tool
      }
    };
    onUpdate(newState);
...
const Whiteboard: React.FC<WhiteboardProps> = ({ state, onUpdate, sessionId }) => {
  const [tool, setTool] = useState<'pen' | 'eraser' | 'rect' | 'circle' | 'text'>('pen');
  const [isDrawing, setIsDrawing] = useState(false);
  const [elements, setElements] = useState<DrawingElement[]>(state?.elements || []);
  const [strokeColor, setStrokeColor] = useState('#000000');
  ...
  const handleMouseDown = useCallback((e: any) => {
    if (tool === 'text') return; // Handle text separately
  ...
  const handleMouseMove = useCallback((e: any) => {
    ...
    setElements(prev => {
      ...
      if (tool === 'pen' || tool === 'eraser') {
        ...
        const radius = Math.sqrt(
          Math.pow(point.x - lastElement.x!, 2) + Math.pow(point.y - lastElement.y!, 2)
        );
        lastElement.width = radius * 2;
        lastElement.height = radius * 2;
        lastElement.x = lastElement.x! - radius;
  ...
  // Handle text input
  const handleTextInput = useCallback((e: any) => {
    if (tool !== 'text') return;
  ...
  }, [tool, elements, strokeColor, saveToHistory, updateWhiteboardState]);
      ...
      case 'line':
        return (
          <Line
            ...
            lineCap="round"
            lineJoin="round"
            globalCompositeOperation={element.stroke === '#FFFFFF' ? 'destination-out' : 'source-over'}
          />
        );
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 bg-white overflow-hidden">
        <Stage
          ...
          height={600}
          onMouseDown={tool === 'text' ? handleTextInput : handleMouseDown}
          onMousemove={handleMouseMove}
          onMouseup={handleMouseUp}
        >
          <Layer>
